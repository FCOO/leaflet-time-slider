...
//*********** NEW CONTROL WITH TIME-SLIDER ***********************
//Create control-box to contain display, buttons and time-slider
var newDatetimeControl  = new L.Control.Box({
			position: 'bottomright',
			icon: 'clock',
			width: 300,		height: 154,
			showText: localLang == 'da' ? 'Vælg tidspunkt' : 'Select time and date',	
			hideText: '',
		})
			.addTo(map);

var $container = $(newDatetimeControl.userContainer);
...
$container.append(
	'<div style="text-align:center; border:1px solid black; padding:0px; margin:0 10px; width:90%; background-color:#DDDDDD;">' +
		'<span id="currentMomentLocal" class="only_local"></span><span class="only_local">&nbsp;('+(localLang=='da'?'lokal':'local')+')</span>' +
		'<span id="currentMomentUTC" class="only_utc"></span><span class="only_utc">&nbsp;('+(localLang=='da'?'UTC':'UTC')+')</span>' +
	'</div>');
...
$buttonContainer.append([
	...
	'<button id="tsNow"		class="'+buttonClass+'"><span class="fa">' + (localLang == 'da' ? 'Nu' : 'Now') + '</span></button>',
...
$container.append('<div style="width:100%; height:80px"><input id="timeSlider"/></div>');

var timeSliderOptions = {
			type	: 'single',		slider: 'small',
			...
			callback: function( result ){ callback( 'datetime', result.fromMoment.toISOString() );
			} 
		}
var timeSlider = $("#timeSlider").timeSlider( timeSliderOptions ).data('timeSlider'),
		$localCheckboxContainer = $('<div style="text-align:left; margin:0; width:100%;"></div>').appendTo($container),
...
$localCheckboxContainer.append([
	$localCheckbox,
	$('<label for="tsLocal">' + (localLang=='da' ? 'Brug lokal tid' : 'Use local time') +' (' + utcOffsetStr + ')' + '</label>')
]);
